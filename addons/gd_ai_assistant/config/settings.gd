...
const DEFAULT_MODE_OVERRIDE: String = "auto"
const DEFAULT_AGENT_MODE: String = "agent"
...
func load_settings() -> void:
	...
	if err != OK and err != ERR_FILE_NOT_FOUND:
		push_warning("GD AI Assistant: could not load settings (error %d). Using defaults." % err)
	_migrate_if_needed()
	_loaded = true
	_dirty = false
...
func save_settings() -> Error:
	...
	if err == OK:
		...
		push_error("GD AI Assistant: could not save settings (error %d)." % err)
	return err
...
func get_active_provider() -> String:
	return str(_config.get_value(SECTION_GENERAL, "active_provider", DEFAULT_PROVIDER_ID))
...
func get_mode_override() -> String:
	return str(_config.get_value(SECTION_GENERAL, "mode_override", DEFAULT_MODE_OVERRIDE))
...
func get_desktop_features_enabled() -> bool:
	...
	return bool(_config.get_value(SECTION_GENERAL, "desktop_features", default_value))
...
func get_fast_mode() -> bool:
	return bool(_config.get_value(SECTION_AGENT, "fast_mode", false))


func set_fast_mode(value: bool) -> void:
	_config.set_value(SECTION_AGENT, "fast_mode", value)
	_dirty = true
...
func get_provider_model(provider_id: String) -> String:
	return str(_config.get_value(SECTION_PROVIDERS, provider_id + "_model", ""))
...
func get_custom_endpoint(provider_id: String) -> String:
	return str(_config.get_value(SECTION_PROVIDERS, provider_id + "_endpoint", ""))
...
func has_any_api_key() -> bool:
	...
	for key: String in _config.get_section_keys(SECTION_PROVIDERS):
		if key.ends_with("_api_key"):
			var value: String = str(_config.get_value(SECTION_PROVIDERS, key, ""))
...
func get_agent_mode() -> String:
	return str(_config.get_value(SECTION_AGENT, "mode", DEFAULT_AGENT_MODE))
...
func get_temperature() -> float:
	return float(_config.get_value(SECTION_AGENT, "temperature", DEFAULT_TEMPERATURE))
...
func get_max_retries() -> int:
	return int(_config.get_value(SECTION_AGENT, "max_retries", DEFAULT_MAX_RETRIES))
...
func _migrate_if_needed() -> void:
	var stored: int = int(_config.get_value(SECTION_GENERAL, "schema_version", 0))
	if stored == 0:
